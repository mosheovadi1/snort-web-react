...
const TextArea = () => {
    ...
    return <>
        <textarea ref={searchInput} type="textarea" name="content" cols="40" rows="10" className="vLargeTextField" maxLength="2048" required="" id="id_content"
            onChange={onchange}
            onFocus={() => setIsFocused(true)}
            onBlur={() => setIsFocused(false)}
            value={content} />
        <label style={{ color: "red" }} title="error">{error}</label>
    </>
}
